...
const MapComponent = ({
  ...
  const MapUpdater = ({ mapRef, setTilesLoaded }) => {
    ...
    useEffect(() => {
      ...
      // Helper: track tile loading on all current/future tile layers
      function setupTileListeners() {
        ...
        map.eachLayer(layer => {
          if (layer instanceof L.TileLayer) {
            ...
            // If tiles are already loaded, force update
            if (layer._tiles) {
      ...
      // Also update listeners when a new tile layer is added
      const onLayerAdd = (e) => {
   ...
   * =================================================================== */
  useEffect(() => {
    if (!tehsilStr || !selectedMauza) {
      setDataMode('shajra');
      return;
    }
    const url = `/JSON Murabba/${tehsilStr}/${selectedMauza}.geojson`;
    fetch(url)
      .then((res) =>
        res.ok && (res.headers.get('Content-Type') || '').includes('application/json')
          ? setDataMode('geojson-only')
          : setDataMode('shajra')
      )
      .catch(() => setDataMode('shajra'));
...
<DrawControl />
        ...
        {dataMode === 'geojson-only' ? (
          <GeoJsonLoader
            geoJsonUrl={`/JSON Murabba/${tehsilStr}/${selectedMauza}.geojson`}
